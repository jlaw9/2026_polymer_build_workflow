...
def assess_ring_piercing(mol : Chem.Mol, conformer_idx : int=0) -> dict[tuple[int], tuple[tuple[int, int]]]:
    ...
    Returns atom indices of each ring, accompanied by all index pairs of 
    bonded atoms which were perceived to pierce the corresponding ring

    Assumes all rings are oblate, i.e. have a minor axis along in which the ring "faces"
    '''
    ...
    return {
        ring_atom_idxs : ring_piercing_idxs(
            positions,
            bonded_atom_idxs,
